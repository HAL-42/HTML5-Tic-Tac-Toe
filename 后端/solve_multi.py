...
def Solve(req_data):
    ...
    x, y = MiniminSearch(np.array(board), difficulty)

    return jsonify({'x': x, 'y': y})


def MiniminSearch(current_state, depth_limit):
    """
    ...
    for action in actions:
        values.append(max_value(action_result(game_state.copy(), action, -1), depth, depth_limit))
...
def min_value(current_state, depth, depth_limit):
    """
    ...
    if depth == depth_limit or abs(current_utility) > 100 or len(actions) == 0:
        return current_utility

    values = []
    for action in actions:
        values.append(max_value(action_result(current_state.copy(), action, -1), depth + 1, depth_limit))

    return (np.array(values)).min()


def max_value(current_state, depth, depth_limit):
    """
    ...
    values = []
    for action in actions:
        values.append(min_value(action_result(current_state.copy(), action, 1), depth + 1, depth_limit))
